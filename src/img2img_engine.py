...
def parse_args():
    ...
    parser.add_argument("--controlnet-type", choices=["canny", "depth", "pose"], default=None, help="Optional ControlNet conditioning type")
...
def main():
    ...
    # Preprocess ControlNet image if requested
    has_controlnet = bool(args.controlnet_type)
    control_image = None
    controlnet = None
    ...
    if has_controlnet:
        ...
        ref_source = args.controlnet_image if (args.controlnet_image and os.path.exists(args.controlnet_image)) else args.image
        print(f"[INFO] Initializing ControlNet (Type: '{args.controlnet_type}', Scale: {args.controlnet_scale}) from '{ref_source}'...")
        raw_control = Image.open(ref_source).convert("RGB")
        raw_control = raw_control.resize((target_w, target_h), Image.Resampling.LANCZOS)
    ...
    mode_label = "Inpainting (Masked)" if has_mask else ("ControlNet Img2Img" if has_controlnet else "Image-to-Image")
    ...
    if has_controlnet:
        ...
        from diffusers import StableDiffusionXLImg2ImgPipeline, StableDiffusionXLInpaintPipeline, UNet2DConditionModel, EulerDiscreteScheduler
        ...
        pipe_lightning_class = StableDiffusionXLInpaintPipeline if has_mask else StableDiffusionXLImg2ImgPipeline
        pipe = pipe_lightning_class.from_pretrained(
            base,
            unet=unet,
            torch_dtype=dtype,
            variant="fp16" if dtype == torch.float16 else None,
